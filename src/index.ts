...
export default {
  async fetch(request: Request, env: unknown, ctx: ExecutionContext): Promise<Response> {
    ...
    let fileFormat: SupportedFormat = 'png';
    const formatMatch = path.match(/\.(webp|gif|jpg|jpeg|png|svg)/i);
    if (formatMatch && isSupportedFormat(formatMatch[1].toLowerCase())) {
      fileFormat = formatMatch[1].toLowerCase() as SupportedFormat;
    ...
    if (DIMENSION_KEYWORDS[lowerDim]) {
      dimensionSegment = DIMENSION_KEYWORDS[lowerDim];
    }
